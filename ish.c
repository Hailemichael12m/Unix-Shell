...
/*this function takes a file to be redirected in and executes 
redirection of input. Rteurns TRUE on success and FALSE in failure.*/
...
/*this function takes a file to be redirected out and executes 
redirection of output. Rteurns TRUE on success and FALSE in failure.*/
...
/*Takes the output of the syntactic analysis and implement 
reditrection using the above two functions defined.*/
...
/*I took the following code from lecture note about linked list*/
...
/*This function executes a command line with single command, which
means command line without pipe. It takes the output of syntactic 
analysyis and executes non-built in functions. Returns TRUE on success
and exits the function with on failure.*/
...
/*This function executes a piped command.It takes the output of 
syntactic analysyis and executes non-built piped in functions. 
Returns TRUE on success and exits the function with on failure.*/
...
while(num_commands < length) {
    char* const* command_array;
    token_types* command_line = (token_types *)DynArray_get(Synout, 
                                    num_commands);
...
/*Signal handler for alarm reset using global variable r*/
static void reset_handler(int isig){
    r = 0;
}
/*Sigquit handler using global variable r*/
...
/*cleanup function adapted from lecture note*/
...
/*Here the main function uses fgets to read inputs and calls the 
above functions for execution*/
int main(){
    int R;
    sigset_t s;
